...
Make sure to remember the name of your class, as you will need it later. It must have the same name as your file. (A file named EsEsProvider.java will have a class `public class EsEsProvider`)
...
### Step 2: Register Your Provider to Both Modloaders

#### NeoForge
Open `neoforge/src/main/java/dev/boxadactle/everyslab/neoforge/EverySlabNeoforge.java` (the mod's main NeoForge class) and find the `GatherDataEvent` handler. It should look something like this:
```java
...
Add a new `event.createProvider(...)` line for your class, exactly as shown above, updating the reference to your class and the locale. 

`NeoLangProvider` is a util class that will allow your localization to run with NeoForge. You must pass in the name of your provider class to point it to your localization code.

#### Fabric
Open `fabric/src/main/java/dev/boxadactle/everyslab/fabric/EverySlabDatagen.java` (the mod's datagen class) and find the `onInitializeDataGenerator` method. It should look something like this:
...
Add a new `pack.addProvider(...)` line for your class, exactly as shown above, updating the reference to your class and the locale.

`FabricLangGenerator` is a util class that will allow your localization to run with Fabric. You must pass in the name of your provider class to point it to your localization code.
